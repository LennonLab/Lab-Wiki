...
if ( !defined( 'MEDIAWIKI' ) ) {
        die( 'This file is an extension to MediaWiki and thus not a 
valid entry point.' );
}
...
$wgExtensionCredits['parserhook'][] = array(
        ...
        'description' => 'Provides a magic word that allows to hide the 
title heading of a page',
  ...
  public static function addMagicWordLanguage( &$magicWords, $langCode ) 
{
  ...
  public static function checkForMagicWord( &$parser, &$text ) {
    if ( isset( $parser->mDoubleUnderscores['notitle'] ) ) {
      $parser->mOutput->addHeadItem( '<style 
type="text/css">.firstHeading, .subtitle, #siteSub, #contentSub, 
.pagetitle { display:none; }</style>' );
    }
    return true;
  }
